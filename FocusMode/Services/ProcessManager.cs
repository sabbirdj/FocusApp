...
public class ProcessManager
{
    ...
    public FocusSession ActivateFocusMode(List<ProcessInfo> toSuspend, List<string> focusAppNames)
    {
        ...
        // Now "ghost" them safely
        foreach (var item in processesToFreeze)
        {
            try
            {
                // Instead of completely freezing the process (which crashes Explorer),
                // we lower its priority to Idle so it uses 0 CPU when other apps are active.
